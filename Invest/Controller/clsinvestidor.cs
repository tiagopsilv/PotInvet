...
namespace Invest.Controller
{
    public class clsinvestidor : ProjetoBLL
    {
        ...
        public void CarregarProjetos(int ID_Projeto = 0, string PerfilProjeto = null, bool Industria = false, bool Varejo = false, bool Artistico = false, bool TI = false, bool Pw = false, bool Video = false, bool Audio = false, string Where = null)
        {
            List<Projeto> _Projetos = new List<Projeto>();
            ListaInvestidor Proj;
            _Projetos = Listar(ID_Projeto, PerfilProjeto, Industria, Varejo, Artistico, TI, Pw, Video, Audio, Where);
            ...
            for (int i = 0; i < _Projetos.Count; i++)
            {
                if (_Projetos[i].Porcentagem < 100)
                {
                    ...
                    Proj.Url = "~/projeto.aspx?ID=" + Convert.ToString(_Projetos[i].ID_Projeto);
