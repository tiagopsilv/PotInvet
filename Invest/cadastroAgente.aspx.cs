...
namespace Invest
{
    public partial class cadastroAgente : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (ClasscadastroAgente.VerificaLogin(cookie.Value.ToString()) == true && Convert.ToBoolean(Session[cookie.Value.ToString()]))
            {
                ...
                ClasscadastroAgente.CarregarListaCusto();

                if (ClasscadastroAgente._ListaCusto.Count > 0)
                {
                    lblTotalCusto.Visible = true;
                    lblTotalCusto.Text = Convert.ToString(ClasscadastroAgente._ListaCusto.Count);
                }

                ClasscadastroAgente.CarregarListaLucro();

                if (ClasscadastroAgente._ListaLucro.Count > 0)
                {
                    lblTotalLucro.Visible = true;
                    lblTotalLucro.Text = Convert.ToString(ClasscadastroAgente._ListaLucro.Count);
                }   
